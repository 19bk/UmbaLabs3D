// ============================================================
// Tiki Tales — Lid + RC522 Holder (v2)
// ============================================================
// v2 change: ADDS a Ø40 × 2mm reinforcement disc on the cavity side
//            of the figurine-dish floor to stop top-layer pillowing
//            during print. Disc is sized to clear all 4 peg positions
//            (nearest peg r=22.7 → 2.7mm clearance at disc r=20).
//
...
rim_h        = 2.0;   // wall height (low — just for PCB alignment)
...
// ---- Dish-floor reinforcement (v2) ----
dish_fill_dia = 40;     // Ø40 keeps 2.7 mm clearance from nearest peg
dish_fill_h   = 2.0;    // thickness added on cavity side
dish_fill_z0  = floor_z - 0.4;  // overlap into floor for clean union

module dish_reinforcement() {
    translate([lid_w/2, lid_d/2, dish_fill_z0])
        cylinder(d = dish_fill_dia, h = dish_fill_h + 0.4);
}
...
union() {
    difference() {
        import("../source/original-lid.stl", convexity = 10);
        cut_old_posts();
    }
    pegs();
    rim();
